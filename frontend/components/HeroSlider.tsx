...
export default function HeroSlider({ banners }: { banners: HeroBanner[] }) {
  ...
  if (!banners.length) return <div className="aspect-[4/3] sm:aspect-[16/7] lg:aspect-[21/9] bg-surface-100" />;
  return <section className="relative overflow-hidden border-b border-surface-300 bg-surface-100" aria-label="Featured collections" onMouseEnter={() => setPaused(true)} onMouseLeave={() => setPaused(false)} onFocus={() => setPaused(true)} onBlur={() => setPaused(false)}>
    <div className="relative aspect-[4/3] sm:aspect-[16/7] lg:aspect-[21/9] max-h-[640px]">
      ...
      {banners.map((banner, index) => <Link key={banner.id} href={banner.link || '/products'} aria-label={banner.altText} className={`absolute inset-0 transition-opacity duration-500 ${index === current ? 'opacity-100 z-10' : 'opacity-0 pointer-events-none'}`}>
        <Image src={banner.imageUrl} alt={banner.altText} fill priority={index === 0} loading={index === 0 ? 'eager' : 'lazy'} sizes="100vw" className="object-cover object-center" onLoad={() => setLoaded(true)} />
      </Link>)}
