...
import { FiGrid, FiUpload, FiEdit2, FiTrash2, FiX, FiPackage, FiInfo, FiCheck } from 'react-icons/fi';
  ...
  return (
    <div className="rounded-2xl border border-surface-200/80 bg-white p-5 sm:p-6 shadow-card space-y-6">
      <div className="flex items-center gap-3 pb-4 border-b border-surface-100">
        ...
        <div>
          <h2 className="text-base font-bold text-surface-900">Category Catalog & Display</h2>
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="text-xs space-y-1 text-surface-700">
            ...
            <p className="text-[11px] leading-relaxed">
              • <strong>Dimensions:</strong> 1920 × 480 px (4:1 widescreen) or 1600 × 400 px.
            </p>
            <p className="text-[11px] leading-relaxed">
              • <strong>File Size:</strong> 150 KB – 400 KB (Max 2 MB). WebP or compressed JPG.
            </p>
            <p className="text-[11px] leading-relaxed">
              • <strong>Alignment:</strong> Center-aligned subjects prevent cropping across both mobile and desktop screens.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-surface-200/80 bg-surface-50/50 p-5 space-y-4">
        <h3 className="text-xs font-bold uppercase tracking-wider text-surface-800">Add New Category</h3>
        ...
        <div className="space-y-3">
          ...
          <div>
            <div className="flex items-center justify-between mb-1">
              ...
              <span className="text-[10px] text-surface-400 font-medium">Rec: 1920 × 480 px</span>
            </div>
            ...
            {bannerImage && (
              <div className="mt-2 rounded-xl border border-surface-200 overflow-hidden bg-black/5 aspect-[4/1] max-h-32 relative shadow-inner">
            ...
            {coverImage && (
              <div className="mt-2 flex items-center gap-3">
                ...
                <span className="text-xs text-emerald-600 font-semibold flex items-center gap-1">
                  <FiCheck className="w-3.5 h-3.5" /> Custom card picture ready
                </span>
              </div>
            )}
          </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-1">
              ...
              <span className="text-[10px] text-surface-400">Appears above footer for brand storytelling</span>
            </div>
            <textarea
        ...
        <div className="admin-sheet-backdrop" onClick={() => setEditingCategory(null)}>
          ...
          >
            <div className="mb-4 flex items-center justify-between pb-3 border-b border-surface-100">
              <h3 className="text-base font-bold text-surface-900">Edit Category</h3>
              <button
            ...
            <div className="space-y-3.5">
              ...
              <div>
                ...
                <div className="flex flex-col gap-2">
                  ...
                  {editBannerImage && (
                    <div className="mt-1 rounded-xl border border-surface-200 overflow-hidden bg-black/5 aspect-[4/1] max-h-28 relative">
                ...
                <div className="flex flex-col gap-2">
                  ...
                  {editCoverImage && (
                    <div className="mt-1 flex items-center gap-3">
                      ...
                      <span className="text-xs text-emerald-600 font-semibold flex items-center gap-1">
                        <FiCheck className="w-3 h-3" /> Custom card image set
                      </span>
                    </div>
                  )}
                </div>
      ...
      {/* Categories List */}
      <div>
        <h3 className="mb-3 text-xs font-bold uppercase tracking-wider text-surface-700">Existing Categories</h3>
        {isLoading ? (
          <p className="text-xs text-surface-400 py-4 text-center">Loading categories...</p>
        ) : categories.length === 0 ? (
          <p className="text-xs text-surface-500 py-4 text-center">No categories created yet.</p>
        ) : (
              ...
              >
                <div className="flex items-center gap-3 overflow-hidden min-w-0">
                  ...
                  <div className="min-w-0">
                    <h4 className="truncate font-bold text-surface-900 text-xs">{c.name}</h4>
                    <div className="flex items-center gap-1.5 flex-wrap text-[10px] mt-0.5">
                      {c.bannerImage ? (
                        <span className="text-accent-600 font-semibold bg-accent-50 px-1.5 py-0.5 rounded">Banner</span>
                      ) : null}
                      {c.description && (
                        <span className="text-emerald-700 font-semibold bg-emerald-50 px-1.5 py-0.5 rounded">Editorial</span>
                      )}
                      {c.hasCustomImage && (
                        <span className="text-surface-600 bg-surface-100 px-1.5 py-0.5 rounded">Card Pic</span>
                      )}
                    </div>
                  </div>
                </div>
